...
- Telegram-авторизация с серверной HMAC-SHA-256 проверкой `initData`, проверкой `auth_date` и подписанной HttpOnly-сессией.
...
Миграции генерируются командой `pnpm db:generate`, применяются только вперёд безопасной командой `pnpm db:migrate`. Railway запускает production runner `node scripts/migrate.mjs` как pre-deploy command. Не используйте `drizzle-kit push` в production.
...
4. Запустите локально с production-переменными:

   ```bash
   pnpm telegram:webhook
   ```

   Скрипт регистрирует `https://<APP_URL>/api/telegram/webhook` с `secret_token` и настраивает Menu Button.
...
9. Один раз выполните `pnpm telegram:webhook` локально с production `.env` либо через Railway shell.
10. Откройте Mini App через бота и проверьте создание → модерацию → публикацию.
...
- Выполнить первый deploy, зарегистрировать webhook и протестировать бот.
