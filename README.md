...
- Иерархический каталог `марка → модель → поколение → год`, каскадные селекты в объявлении и фильтры с сохранением в URL.
- Админка каталога: создание, редактирование, деактивация, алиасы, безопасное слияние, очередь пользовательских исправлений и журнал импортов.
...
Миграции генерируются командой `pnpm db:generate`, применяются только вперёд безопасной командой `pnpm db:migrate`. Railway перед каждым deploy автоматически применяет миграции и проверяет Telegram webhook. Массовый импорт каталога при старте production-сервиса намеренно не запускается. Не используйте `drizzle-kit push` в production.

## Каталог автомобилей

Список из 163 марок и приоритетные марки закреплены требованиями проекта. Модели и поколения получены консервативным структурированным запросом к Wikidata; данные Wikidata доступны по лицензии CC0 1.0. Неизвестные годы и коды остаются `null`, коммерческие сайты не скрейпятся. Это проверяемая открытая база, но не заявление о полном покрытии всех автомобилей мира.

```bash
# Проверка файла без подключения к БД
pnpm catalog:validate -- --offline

# План импорта без записи в БД
pnpm catalog:import -- --dry-run --offline

# Импорт/обновление в настроенную PostgreSQL
pnpm db:migrate
pnpm catalog:import

# Статистика и проверка ссылочной целостности БД
pnpm catalog:validate
pnpm catalog:stats
```

Повторный импорт той же версии ничего не дублирует и завершается статусом `already_imported`. Для осознанного повторного применения версии предусмотрен `pnpm catalog:import -- --force`. Отчёт каждого выполненного импорта сохраняется в `vehicle_catalog_imports`, включая результаты привязки старых текстовых объявлений. Подробности источника и ограничения находятся в `data/catalog/README.md`.

После первого Railway-деплоя импорт нужно запустить один раз вручную в shell сервиса: `./node_modules/.bin/tsx scripts/catalog-import.ts`. Файл данных и импортёр включены в production-образ, но при обычном старте приложения команда не выполняется.
